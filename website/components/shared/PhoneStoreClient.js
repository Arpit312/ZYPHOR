    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          <Search className="absolute left-4 top-3.5 h-4 w-4 text-black/30" />
          <input
            type="text"
            value={query}
            onChange={e => setQuery(e.target.value)}
            placeholder="Search iPhone, Samsung, OnePlus..."
            className="w-full bg-white border border-black/10 rounded-xl pl-11 pr-4 py-3 text-sm text-slate-850 placeholder-black/30 focus:outline-none focus:border-coral transition-colors shadow-sm"
          />
        </div>
        <select
          value={sortBy}
          onChange={e => setSortBy(e.target.value)}
          className="bg-white border border-black/10 text-slate-850 text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-coral cursor-pointer shadow-sm"
        >
          ...
          <button
            ...
            className={`shrink-0 px-4 py-2 rounded-xl text-xs font-semibold font-mono transition-all ${
              activeBrand === b
                ...
                : "bg-white text-black/50 border border-black/10 hover:text-slate-850 hover:border-black/30 shadow-sm"
            }`}
          >
            {b}
          </button>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <span className="text-black/40 text-xs font-mono uppercase tracking-wider">Condition:</span>
        {conditions.map(c => (
          <button
            key={c}
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              activeCondition === c
                ...
                : "bg-white text-black/50 hover:bg-black/5 hover:text-slate-850 border border-black/10"
            }`}
          >
            {c}
          </button>
        ))}
        <span className="ml-auto text-black/30 text-xs font-mono">
