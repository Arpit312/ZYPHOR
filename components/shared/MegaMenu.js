...
export default function MegaMenu() {
  ...
  return (
    <div className="relative" onMouseEnter={openMenu} onMouseLeave={closeMenu}>
      <button className="flex items-center gap-1 px-3 py-2 text-sm rounded-md text-white/70 hover:text-white hover:bg-white/5 transition-colors focus-ring">
        Buy Phones <ChevronDown className={`h-3.5 w-3.5 transition-transform ${open ? "rotate-180" : ""}`} />
      </button>

      {open && loaded && (
        <div className="absolute top-full left-0 mt-1 w-[640px] max-w-[90vw] bg-white rounded-2xl border border-black/[0.08] shadow-2xl overflow-hidden flex">
          {menu.length === 0 ? (
            <div className="p-8 text-center w-full">
              <Smartphone className="h-10 w-10 text-black/10 mx-auto mb-3" />
              <p className="text-sm text-black/40">No listings yet. Be the first to list a device!</p>
